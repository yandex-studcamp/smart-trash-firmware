...
#include "camera/smart_camera.hpp"
#include "inference/smart_inference.h"
#if CONFIG_SMART_ENABLE_NETWORK_DEBUG_API
#include "esp_netif.h"
#include "network/smart_http_server.hpp"
#include "network/smart_softap.hpp"
#endif
#endif
...
constexpr char kTag[] = "smart_boot";
...
void run_boot_camera_inference()
{
    ...
    const esp_err_t infer_init_ret = smart_inference_init();
    ...
    const esp_err_t cam_init_ret = smart_bin::smart_camera_init();
    ...
    const esp_err_t capture_ret =
        smart_bin::smart_camera_capture_rgb888(&rgb_data, &rgb_width, &rgb_height, &capture_ms);
    if (capture_ret != ESP_OK) {
        ESP_LOGE(kTag, "Camera capture failed: 0x%x", capture_ret);
        smart_bin::smart_camera_deinit();
        return;
    }
    ...
    const esp_err_t infer_ret = smart_inference_run_rgb888(rgb_data, rgb_width, rgb_height, &result);
    const float infer_call_ms = static_cast<float>(esp_timer_get_time() - infer_call_start_us) / 1000.0f;

    smart_bin::smart_camera_free_rgb888(rgb_data);
    smart_bin::smart_camera_deinit();
    ...
    ESP_LOGI(kTag,
             ...
             result.confidence);
    ESP_LOGI(kTag, "Scores: paper=%.4f plastic=%.4f other=%.4f", result.scores[0], result.scores[1], result.scores[2]);
    ESP_LOGI(kTag,
